...
>
> **REVISÃO 81 — 26/08/2026.** Se o número aqui não for o mais alto que você
...
- **Frontend (`page-mega-results`):** duas sub-abas. **Importar** — Conexão,
  Importar relatório (upload + progresso por polling em `import_batch`),
  Importações recentes; sem mudança desde a REVISÃO 78. **Métricas (REVISÃO
  80, NÃO DEPLOYADA/NÃO MEDIDA)** — seletor de período, 8 KPIs com comparação
  vs. período anterior, gráfico de comissão por dia, breakdown por loja;
  consome `mega-results/metrics/query` pela primeira vez desde que a função
  existe (08/08). ⚠️ Código no repo, sem prova por comportamento — o sandbox
  desta sessão não alcança o Supabase nem o EasyPanel.
