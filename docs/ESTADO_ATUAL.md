...
Branch `redesign/megalinks-ui-v3`, 29 commits à frente da `main` (28 + merge), 0 atrás em 19/09, working tree limpa, enviada ao remoto. Cada commit testado individualmente via `frontend-static` (porta 5173) — console sem erro novo, foco de teclado conferido ao vivo com Tab de verdade num modal real.
...
5. **Em aberto:** (a) conferir com login real — KPIs do admin, tela de trial encerrado, `revops.html` logado, telas com dados reais em mobile; (b) `#modalTicket` e `#modalNewPass` no modal antigo, fora do sistema de foco (o de senha é recuperação: não deve fechar por engano); (c) `.btn.loading` — nenhum código aplicaria a classe hoje; (d) performance (bundle/fontes) não analisada; (e) ~7 fundos escuros de badge com hex solto em `revops.html`; (f) abrir PR / mergear em `main` e deploy no EasyPanel — do Érico.
