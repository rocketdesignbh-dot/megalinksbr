...
- **Performance (etapa 14), MEDIDA em produção:** `GET /painel/` devolve **843.735 bytes sem `Content-Encoding`** (o `nginx:alpine` traz o gzip desligado; `gzip -9` do mesmo arquivo dá ~244 KB). Corrigido em `frontend/Dockerfile` (`6e56356`): `gzip on` para css/js/json/svg/texto no `server`, e `gzip off` em `/r/` (não mexer no que a P60 mediu). ⚠️ **Sintaxe do nginx NÃO validada** (sem docker/nginx na máquina): depois do rebuild no EasyPanel, conferir `Content-Encoding: gzip` em `/painel/`.
