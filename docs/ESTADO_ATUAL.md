...
>
> **REVISÃO 167 — 25/09/2026.** Se o número aqui não for o mais alto que você
...
**REVISÃO 167 — 25/09/2026 (manhã) — Post Automático passa a postar produto do Mercado Livre com o link nativo `meli.la` (`send-post` v34, deploy 69, `verify_jwt:true`, conferida byte a byte). Diagnóstico dos cupons de hoje. Nada do Clone Post (`clone-ingest`/`resolve-link`) foi tocado.**

### 1. Por que o ML "voltou" a sair com `megalinksbr.com.br/r/` (MEDIDO)
- Os links do ML que saíram com o encurtador hoje vieram do **Clone Post**: a vitrine (`resolve-link` v6, REVISÃO 166) passou a trazer produto de ML — `clone_posts` de ML aprovados: **0 antes de 25/09, 3 em 25/09** (todos da fonte `43f7fde4…`). Exemplos: Creatina Dark Lab (`/r/53ZrNSa`, 08:19 BRT) e Tênis New Speed (`/r/KY35cuS`, 10:00 BRT).
- Esses produtos saem pelo `send-post`, e o `send-post` **nunca** usou o link nativo do ML: decisão do Érico em 14/09 (REVISÃO 149/P146, "só Shopee no automático", pelo risco do cookie). O link nativo do ML só existia no Postar Agora e no Link Rápido. Não era regressão — era produto de ML chegando por um caminho que antes não chegava.
- **Decisão do Érico em 25/09: ML com `meli.la` também no automático.**

### 2. O que mudou — `send-post` v34
- `gerarLinkNativoML()` (cópia do endpoint da `product-search`: `POST /affiliate-program/api/v2/stripe/user/links`, cookie `profiles.ml_session_cookie` + `_csrf` + Etiqueta ML do dono do grupo), **uma chamada por post de ML**; o cookie só é lido quando o produto da vez é de ML. Tira `matt_*` da URL antes (mantém `pdp_filters`).
- `ehLinkNativoLoja()` = Shopee nativo **ou** `meli.la` → sai "cru", sem o encurtador.
- Falhou (sem cookie/etiqueta, cookie vencido, item inelegível 111, timeout 12 s) → fluxo de sempre: `matt_*` + `/r/`. Cupom de ML não mudou (continua `matt_*` + encurtador). `group-blast` (Disparo Manual) não mudou.

### 3. Cupons (pergunta do Érico: "teve cupom nos grupos clonados e não foi repostado") — MEDIDO
- `coupon_captures` de 25/09: **2**, ambas do mesmo texto ("R$10 OFF acima de R$199", `s.shopee.com.br/50ZKzs6rds` → `/m/cupom-de-desconto`).
  - **Achadinhos Geral:** capturado 08:19 BRT, **`sent` às 10:00:02 BRT** (horário de cupom das 10h); o Storage registrou a leitura da imagem da Shopee às 10:00:27. Chegada no WhatsApp não conferida por mim.
  - **"teste cupom":** `approved`, mas o grupo está com **Post Automático desligado** (`post_auto_enabled=false`) — o `send-post` nem olha esse grupo. Não sai enquanto o Post. Automático dele estiver desligado.
- Nenhum outro cupom foi capturado em 30 h. O que não virou cupom: `meli.la` nas fontes sem ML em `lojas_permitidas` (descartado no pré-filtro, antes de saber se é cupom), `blz.to` (encurtador responde 403) e `linktr.ee`.

### Prova
- **Cookie do Érico válido hoje:** a mesma chamada, feita pelo `pg_net` com o cookie e a Etiqueta `eko`, para o Tênis `MLB-5328448952` → **200, `short_url: https://meli.la/1M2ry4N`** (o `long_url` do ML aponta para `/social/<afiliado>` com `matt_word=eko&matt_tool=18051249`).
- Deploy 69: `get_edge_function` **igual ao repo** (index.ts e deno.json), `verify_jwt:true`, `import_map_path` `deno.json`. Cron `mega-send-post` **200** às 13:29 e 13:31 UTC, depois do deploy. `esbuild` limpo.
- ⚠️ **NÃO MEDIDO:** um post real de ML saindo com `meli.la` num grupo — não havia produto de ML na fila de nenhum grupo com Post Automático ligado às 10:30 BRT. Ver P168.

---

**REVISÃO 166 — 24–25/09/2026 — "Grupo de Achadinhos #14" sem postar (diagnóstico) + CUPONS AUTOMÁTICOS (função nova, pedido do Érico) + VITRINE DO ML VIRA PRODUTO (`resolve-link` v6, deploy 14). `clone-ingest` v33 (deploy 33) e `send-post` v32 (deploy 67) NO AR, as duas conferidas byte a byte contra o repo. Migração `cupons_automaticos` aplicada. Painel: seção nova em Cupons. **Depois, no mesmo dia: cupons também para Amazon e Mercado Livre — `clone-ingest` v35 (deploy 35) e `send-post` v33 (deploy 68) NO AR, conferidas byte a byte (ver seção 5).**
...
- **`frontend/index.html`** (página Cupons): card "📣 Cupons automáticos dos grupos" — escolhe o marketplace cadastrado (`affiliate_credentials`), sobe 1 imagem, 1x/2x/3x por dia, "⚡ Sai automático" / "✋ Eu aprovo antes", liga/desliga (só liga com imagem), lista dos cupons capturados nos últimos 3 dias com Aprovar/Descartar. Shopee, Amazon e Mercado Livre capturam (seção 5); as outras lojas mostram "ainda não disponível". Dica de imagem no card: quadrada 1080×1080 px, JPG/PNG/WEBP até 5 MB. Rótulos novos no card da fonte do Clone Post.

### 4. Vitrine do afiliado no ML vira produto — `resolve-link` v6 (deploy 14, `verify_jwt:true` como estava)
- **Problema MEDIDO:** 256 recusas `resolve_falhou` de `/social/<afiliado>` em 7 dias (maior perda de ML do Clone Post; fontes do Érico e da outra cliente `08a269c8…`).
- **Descoberta MEDIDA (25 links `meli.la` reais, 25 de 25):** a página `/social/<nome>?ref=…` traz o produto compartilhado em DESTAQUE — `"item_id":"MLB…"` junto de `"source":"affiliate-profile"` (um por página) + o card `"metadata":{"id","product_id",…,"url"}` desse item; `og:title` = título do produto. Página que é só LISTA do afiliado ("Minhas listas de recomendações") não tem a marca.
- **Código:** `produtoDaVitrine()` abre a vitrine (com o `ref`, antes do `stripAffiliate`), exige as DUAS marcas batendo (não adivinha pelo nome — há anúncios quase homônimos na mesma vitrine) e devolve a página do produto (`/p/…` ou `/up/…` + `?pdp_filters=item_id%3AMLB…`, `produto.mercadolivre.com.br/MLB-…` quando for esse o card). Lista → `stage:"vitrine"` com motivo próprio. Resposta ganha `vitrine:{de,item,titulo}`.
- **Prova:** `resolve-link` publicada, 6 links: 5 viraram o anúncio certo (título conferido), 1 lista recusada. `clone-ingest` `dryRun` na fonte "Eletrodomésticos #132" (Érico, todas as lojas): TV Philips (`/p/`) → `salvaria`, R$ 3.198,89 de R$ 4.895, foto e `affiliate_url` com `matt_tool` do Érico; lava-louças (`/up/`) → 1ª tentativa `sem_imagem` (leitura do ML falhou), 2ª → `salvaria` R$ 1.931,08 de R$ 2.209,48 com foto — **a leitura do ML oscila, não o formato**; lista → `resolve_falhou` "LISTA do afiliado". Nada gravado.
- ⚠️ Só vitrine "ofertinhas" medida (a que domina o log). Outras vitrines podem ter layout diferente — se a marca faltar, a mensagem volta a ser recusada, não publicada errada. Fonte com ML fora do `lojas_permitidas` (ex.: "Grupo de Achadinhos #14") continua descartando `meli.la` no pré-filtro, como antes.
- ⚠️ `get_edge_function` da `resolve-link` conferido visualmente contra o repo (não byte a byte por script — a função é pequena e o retorno veio inline).

### 5. Cupons também Amazon e Mercado Livre (25/09, pedido do Érico)
- **`clone-ingest` v34→v35 (deploy 35, `verify_jwt:false`):** `lojaDoCupom()` reconhece — Amazon (`amazon.com.br`): `/promotion/…`, `/coupons`, `/cupons`, `/deals`, `/gp/goldbox`; Mercado Livre (`mercadolivre.com.br`): `/cupons…`, `/ofertas…`, `/l/<campanha>`. `coupon_captures.store` usa a chave de `coupon_settings` (`shopee`|`amazon`|`mercado_livre`). Rótulos: "Promoção Amazon", "Ofertas do dia", "Cupons de desconto". v35: o nome da loja nunca vira código ("Cupom Amazon: 20% OFF" dava `coupon_code=AMAZON`) e o texto do log diz "do Mercado Livre".
- **`send-post` v33 (deploy 68, `verify_jwt:true`, `import_map_path` `deno.json`):** `linkAfiliadoCupom()` por loja — Shopee igual à v32 (`generateShortLink`, sub-ids `[Sub-ID, "cupom"]`); Amazon = `tag` do "ID de Associado" na URL da promoção + encurtador próprio; ML = `matt_tool`/`matt_word`/`matt_medium` + encurtador próprio. **Sem a credencial da loja o cupom não sai.** Título: "🎟️ CUPONS {SHOPEE|AMAZON|MERCADO LIVRE} LIBERADOS 🎟️".
- **Frontend:** `CUP_AUTO_SUPORTADAS` = shopee, amazon, mercado_livre; dica do tamanho da imagem.
- **Origem MEDIDA:** Amazon `/promotion/psp/…` aparece nos grupos reais (8 recusas "não achei o ASIN" em 14 dias). **Mercado Livre: nenhum link de cupom nos grupos em 30 dias** — os caminhos entraram pela estrutura pública do site, não por medição.
- **Prova (dryRun, fonte `67285808…` do Érico, todas as lojas, `coupon_settings` Amazon+ML temporários):** `/promotion/psp/A22OO7J1WYB7J8` → `cupom_salvaria` (Promoção Amazon; na v35, código nulo para "Cupom Amazon:" e `LIVROS20` para "Use o cupom: LIVROS20"); `mercadolivre.com.br/cupons` → `cupom_salvaria` (Cupons de desconto, `MELI10`); link de produto Amazon seguiu o caminho de produto. Config temporária apagada; `coupon_captures`/log de teste = 0. Cron `mega-send-post` 200 a cada minuto depois do deploy 68. O `coupon_settings` Shopee do Érico (ativo, 3x, automático) não foi tocado.
- ⚠️ **NÃO MEDIDO:** atribuição de comissão da Amazon (`tag`) e do ML (`matt_*`) em página de promoção/cupom — só em produto está medido. Nenhum cupom Amazon/ML saiu num grupo ainda.
...
**✅ RESOLVIDO no mesmo dia (merge feito, ver acima). O que segue é o registro do que foi medido ANTES do merge, via CLI (`git fetch` + `git merge-tree`, simulação sem tocar em nada) — a branch estava DEFASADA em relação à `main`:**
...
- ~~Branch não mergeada em `main`~~ — **superado em 19/09:** o PR #19 foi mergeado em `main` (`022f63b`, 14:17 UTC, merge commit). Na hora em que esta linha foi escrita eram 29 commits à frente e sem PR. O **deploy** (rebuild no EasyPanel) segue decisão do Érico.

### Estado ao encerrar

Branch `redesign/megalinks-ui-v3`, 29 commits à frente da `main` (28 + merge), 0 atrás em 19/09, working tree limpa, enviada ao remoto. **Atualização (2ª sessão de 19/09): a branch chegou a 35 commits à frente, 0 atrás, e o PR #19 foi MERGEADO em `main` (`022f63b`) — ver o adendo acima do handoff.** Cada commit testado individualmente via `frontend-static` (porta 5173) — console sem erro novo, foco de teclado conferido ao vivo com Tab de verdade num modal real.

**Adendo de 19/09 (2ª sessão do dia) — PR #19 MERGEADO em `main` (`022f63b`), gzip revertido, revalidado e mergeado à parte como PR #20 (`39fa036`) e **JÁ NO AR em produção**, `#modalTicket` migrado, conferência logada PARCIAL (commits `6e56356` → revertido em `07fcbd3`, e `b05bafb`).**

- **PR #19 mergeado:** [rocketdesignbh-dot/megalinksbr#19](https://github.com/rocketdesignbh-dot/megalinksbr/pull/19), `redesign/megalinks-ui-v3` → `main`, **merge commit `022f63b` em 19/09 às 14:17 UTC** (35 commits; `mergeable: MERGEABLE`, `CLEAN`, sem checks configurados; a `main` não tinha andado: `0 35` no `rev-list` logo antes). A branch `redesign/megalinks-ui-v3` **não foi apagada** no remoto *(superado: foi apagada depois, a pedido do Érico; ver o adendo "4ª parte")*. **O deploy é AUTOMÁTICO (medido em 19/09):** logo depois do merge do #20 (14:32:58 UTC), às 14:33:11 UTC produção já servia o `Dockerfile` novo (gzip) e o HTML do #19 (`class="mbg" id="modalTicket"` presente no `/painel/` servido). O push/merge na `main` dispara o rebuild sozinho — o histórico antigo ("foi o Érico quem rebuildou") não vale mais para este fluxo. Não medi quanto tempo o rebuild leva de fato: 13 s após o merge já estava no ar, mas o #19 foi mergeado 15 min antes e pode ter sido publicado nesse intervalo.
- **Verificação antes do PR:** smoke `node tools/smoke-index.mjs` = 5 blocos, 1 erro (o mesmo `includes` com regex), **idêntico ao baseline de `origin/main`**. Overflow: 28 telas × 320/375/414/768px = **0 estouros** (layout estático, sem login).
- **Performance (etapa 14), MEDIDA em produção:** `GET /painel/` devolve **843.735 bytes sem `Content-Encoding`** (o `nginx:alpine` traz o gzip desligado; `gzip -9` do mesmo arquivo dá ~244 KB). A correção foi escrita em `frontend/Dockerfile` (`6e56356`: `gzip on` para css/js/json/svg/texto no `server`, e `gzip off` em `/r/` para não mexer no que a P60 mediu), mas **foi REVERTIDA antes do merge do PR #19** (`07fcbd3`, decisão do Érico): a **sintaxe do nginx não foi validada** (sem docker/nginx na máquina) e uma config inválida derruba o frontend inteiro no rebuild. **Foi para produção como PR separado: [#20](https://github.com/rocketdesignbh-dot/megalinksbr/pull/20), MERGEADO (merge commit `39fa036`, 19/09 às 14:32 UTC; aberto como rascunho e marcado como pronto depois da validação)** (branch `perf/nginx-gzip-frontend`, criada da `main` com `git cherry-pick 6e56356` → `8e2056d`; sem Docker e sem WSL na máquina). **VALIDADO em 19/09** com o **nginx 1.31.6 oficial para Windows** (mesmo número de versão que a produção anuncia em `Server:`), baixado de `nginx.org/download/` (2.797.070 bytes, conferido contra o `Content-Length`) para a pasta temporária da sessão, sem instalar nada no sistema; o `default.conf` foi extraído de dentro do Dockerfile e incluído num `http{}` de teste:
  - **`nginx -t`:** a config do PR passa (`syntax is ok`, exit 0); o controle com a config atual da `main` também passa; o **controle negativo** (uma diretiva inválida injetada) reprova com `unknown directive`, exit 1 — ou seja, o teste discrimina.
  - **Teste funcional** (nginx numa porta de teste, servindo o `frontend/` real, `Accept-Encoding: gzip`): `/index.html`, `/painel/`, `/painel/dashboard`, `onboarding.css` e `exemplo-postagens.js` saem com `Content-Encoding: gzip` e `Vary: Accept-Encoding`; `icon.png` sai sem gzip; `/painel/` cai de **866.637 para 244.946 bytes (-72%)**; o corpo descomprimido é **idêntico** ao `index.html`; o `Cache-Control: no-cache, must-revalidate` do P49 foi preservado.
  - **Não testado:** `location /r/` (proxia para o Supabase de produção; só validado por sintaxe, com `gzip off`) e o build **Alpine** em si — a validação foi no build Windows, com as mesmas diretivas.
  - **CONFERIDO EM PRODUÇÃO (19/09, 14:33 UTC, ~13 s após o merge):** `/painel/` responde com `Content-Encoding: gzip` e `Vary: Accept-Encoding`: **243.869 bytes com gzip contra 852.567 sem** (~-71%; antes eram 843.735 sem compressão nenhuma), e o corpo descomprimido tem o **mesmo md5** do corpo sem compressão. `/`, `/guia`, `/termos.html`, `/privacidade.html` e `/painel/dashboard` respondem 200 com gzip e o `Cache-Control: no-cache, must-revalidate` preservado; `onboarding.css` e `exemplo-postagens.js` saem com gzip; `icon.png` sem.
  - **`/r/` (o ponto que a validação local não cobria):** responde `Content-Encoding: gzip` mesmo com o `gzip off` — **não é o nosso nginx comprimindo**: o Supabase/Cloudflare já devolve gzip quando recebe `Accept-Encoding: gzip` (medido direto em `nxlfezpagporealqqbfj.supabase.co`, `Server: cloudflare`) e o nginx apenas repassa; sem pedir compressão a resposta vem sem `Content-Encoding`, e com `--compressed` o corpo decodifica. É o mesmo comportamento de antes do PR. **Limite:** testado só com um código inexistente (404); **não** exercitei um redirect real nem o preview de link do WhatsApp (tags OG).
  - **Achado NÃO aplicado, de propósito:** 4 `<script>` síncronos no `<head>` (`supabase-js` via CDN, `onboarding.js`, `exemplo-postagens.js`, `exemplo-configuracao.js`). Trocar por `defer` **quebra o app**: os scripts inline rodam no parse e usam o global `supabase`. Fontes já têm `preconnect` e `display=swap` — sem ganho barato.
  - `boas-vindas-pc.png` / `-cel.png` (1,4 MB cada) só são copiadas no Dockerfile; nenhuma página as referencia (provável uso em e-mail).
- **`#modalTicket` (`b05bafb`):** migrado para `.mbg > .modal > .mh + .mb`; `abrirTicket` usa `openModal`/`closeModal`. Testado com ticket **simulado** (stub de `SB.rpc`): foco entra, Tab/Shift+Tab presos, ESC/clique no fundo/× fecham e devolvem o foco ao botão de origem. **Não foi visto com ticket real.** `#modalNewPass` continua no modal antigo, de propósito.
- **Reduced-motion (etapa 12), observado até onde deu:** o bloco `@media (prefers-reduced-motion: reduce)` (`index.html:79`) foi extraído do CSS carregado e aplicado sem a condição: durações computadas de modal/backdrop/dropdown caem de 0,18/0,14/0,14 s para 0,01 ms. **Não** observei o navegador casando a media query (o `emulate` do chrome-devtools não tem essa opção e não se deve mexer na config de animações do Windows).
- **Conferência logada (Érico logado como admin em `localhost:5173`), só olhando:**
  - Visão Geral do admin abre com dados reais, sem overflow horizontal.
  - 🔴 **Achado corrigido (`b05bafb`):** o gráfico "Distribuição de planos" ainda usava `#10B981` e `#3B82F6` (Tailwind) em Starter/Pro; Elite/Premium já estavam dessaturados. Agora `var(--green)` e `#7C9EC9`; defaults dos planos alinhados. Conferido: sem Tailwind restante na Visão Geral.
  - ❓ **Observação, não causada pela branch (só troquei cores):** a Visão Geral mostrou "Usuários ativos 0" e "MRR R$ 0,00" ao lado de 15 usuários e distribuição de planos preenchida. Vale conferir se é esperado.
  - **Não conferido:** tela de trial encerrado, `revops.html` logado, telas com dados reais em 375px, `#modalTicket` com ticket real.
- **Lições operacionais desta sessão:**
  - `serve` (porta 5173) **não tem fallback de SPA**: abrir `/painel/adm-visao` direto dá 404. Entrar por `/index.html` e clicar em **Admin** no seletor. A porta 5173 pode estar ocupada por outro chat, mas serve o mesmo `frontend/` (conferido com `cmp`).
  - O storage do Chrome do grupo de abas do Claude era **separado** do Chrome do Érico: a sessão só existiu depois de ele clicar em "Entrar" na aba aberta pelo agente. Login/senha ficam com o Érico.
  - `fetch('http://localhost:5173')` a partir da aba de produção ficou pendente (provável prompt de rede local do Chrome); não contornar.
  - `javascript_tool` do Chrome devolve `{}` para `Promise`/`setTimeout`: usar código síncrono e esperar com `computer wait`.
  - O hook GateGuard bloqueia **só a primeira** edição de um lote paralelo e deixa as outras passarem, o que deixa o arquivo pela metade. Editar em sequência e conferir com `git diff`.

**Adendo de 19/09 (3ª parte) — PASSADA LOGADA EM PRODUÇÃO (Chrome com a sessão do Érico; só leitura).** Nenhum problema encontrado nas telas medidas.

- **Como foi feito:** aba do Chrome do agente em `https://www.megalinksbr.com.br/painel/` (sessão já ativa); navegação por `a[data-page]` e por `go('sec',this)` (revops) via JS; um script em segundo plano medindo, por tela: overflow horizontal, restos das paletas retiradas (lista de `rgb()` exatos), "Carregando" preso e erros de JS (`error` / `unhandledrejection`). Nenhuma ação que grave: nunca acionei "Resolver", "Salvar" nem "Conceder".
- **Código novo no ar:** `#modalTicket.mbg` e a regra `:focus-visible` presentes no HTML servido e na página.
- **Desktop (viewport 1568 px):** afiliado **17 telas** + admin **8 telas** = 0 overflow, 0 resto de paleta, 0 "Carregando" preso, 0 erro de JS. **Pulada:** `conexao` (Conexão WhatsApp), porque podia iniciar um pareamento.
- **Mobile 371 px, com dados reais:** a janela do Chrome **não estreitou** (`resize_window` não muda o viewport reportado: continuou 1568 px). Solução: **iframe da mesma origem de 375 px** carregando o próprio `/painel/` (viewport 371 px, media queries 768/520 ativas, sessão compartilhada). As mesmas **25 telas** = 0 overflow, 0 resto de paleta. **Não é um celular de verdade.**
- **Visão Geral do admin:** o gráfico de planos aparece nas cores novas (verde, azul `#7C9EC9`, violeta, âmbar), sem Tailwind.
  - **"Usuários ativos 0 / MRR R$ 0,00 / Em trial 0" está EXPLICADO, não é bug:** `renderAdminVisao` conta só `payingUsers = USERS.filter(u => !u.is_vip && !u.is_admin)` com `status` active/trial; a distribuição de planos conta **todos** os `USERS` (15). O gráfico de MRR e as linhas de tendência são **placeholders com zeros fixos** (`z`) — já era assim antes da branch.
- **`#modalTicket` com ticket REAL** (3 tickets: 1 aberto, 2 resolvidos): abriu pelo botão "Ver"; `role="dialog"`, `aria-modal`, `aria-labelledby`, foco em "Fechar", e a RPC `get_user_email` devolveu o e-mail. **Teclas reais:** 3 Tabs chegam a "Resolver" (o último), o 4º volta a "Fechar"; ESC fecha e devolve o foco ao botão "Ver". **Nenhum dado alterado** (1 aberto / 2 resolvidos antes e depois; nunca pressionei Enter/Espaço em "Resolver").
- **`revops.html` logado** (mesma sessão, `appView` visível): **9 seções × 1200 e 375 px** = 0 overflow, 0 resto de paleta. **Puladas por risco de efeito colateral:** IA Insights (pode chamar IA, com custo), Automações e Conexões Admin.
- **Trial encerrado:** sem conta com trial expirado, o template foi **extraído de `origin/main`** (`document.body.innerHTML` em `enterApp`, ~linha 12624) e renderizado num **iframe descartável**, com os `onclick` removidos. Tokens aplicados (título e CTA `rgb(255,193,7)`, texto `rgb(12,12,14)`, fonte Archivo, ícone carregado), 0 overflow em 1196 e 371 px. **Não é o fluxo real** que dispara essa tela.
- **Limites e lições:**
  - `captureScreenshot` do Chrome MCP estourou 30 s **4 vezes** (janela 2560×911; com o modal aberto e o fundo desfocado; com o iframe do trial). Só funcionou logo depois de `resize_window` e sem modal. A verificação visual ficou por **DOM e estilos computados**, sem captura do modal nem do trial.
  - `find` lê a árvore de acessibilidade **incluindo o gate de login escondido**: inútil aqui. Clique por coordenada errou o alvo; usar `.click()` no elemento.
  - O detector de paleta só acha os `rgb()` exatos da lista; uma cor nova fora dela não apareceria.
- **Segue SEM teste:** redirect `/r/` real e preview de link do WhatsApp (só o 404 foi visto); comportamento com reduced-motion ligado; trial encerrado real; as 4 páginas puladas acima. *(Superado em parte pelo adendo "4ª parte" abaixo: o `/r/` real e a prévia OG foram medidos.)*

**Adendo de 19/09 (4ª parte) — `/r/` REAL e PRÉVIA OG do WhatsApp MEDIDOS em produção depois do gzip (PR #20); branch `redesign/megalinks-ui-v3` APAGADA no remoto. Nada de código mudou.**

- **Como foi feito (só leitura, sem login):** `curl` em `https://www.megalinksbr.com.br/r/<código>` sem seguir redirect, **sempre com User-Agent de robô de prévia** (`WhatsApp/2.2631.102 W`, `facebookexternalhit/1.1`). Escolha deliberada: a `redirect` v16 só grava `link_clicks`/`increment_clicks` para gente (`ROBO_DE_PREVIA`), então nenhuma requisição desta passada contou clique. **Não** usei `curl` puro nem User-Agent de navegador (contariam clique humano nos links reais do Érico).
- **Redirect real (caminho de fallback 302, links sem `og_title`):**
  - `xzadtgh` (Shopee) → `302`, `Location: https://s.shopee.com.br/an_redir?origin_link=…&affiliate_id=18344180897&sub_id=xzadtgh` (o `sub_id` da P62 continua gravado e viajando).
  - `d9q7va7` (Amazon) → `302`, `Location: https://www.amazon.com.br/dp/B0C9W3MZV2?tag=eko04e-20` (destino intacto, sem `sub_id`, como na medição de 17/08).
  - Nos dois: `Content-Length: 0`, `Content-Type: text/html; charset=utf-8` (o `add_header` do nginx atua também no 302, inofensivo, como o comentário do P60 previa) e `Server: nginx/1.31.6`.
- **Prévia própria (caminho HTML, link com `og_title`):** contra `zzogt01`, o mesmo registro de teste da REVISÃO 50 (título "TESTE OG — Sabonete Johnson's Baby 200mL", destino = home).
  - `WhatsApp/2.2631.102 W` e `facebookexternalhit/1.1` → `200`, `Content-Type: text/html; charset=utf-8`, `Cache-Control: public, max-age=600`; **sem** `x-content-type-options: nosniff` e **sem** CSP `sandbox` (o `proxy_hide_header` do P60 segue funcionando depois do gzip).
  - Corpo com todas as tags: `og:type/site_name/title/description/image/image:alt`, `twitter:card/title/description/image`, `robots noindex,nofollow`, `http-equiv=refresh` e `location.replace`. **Escape correto** de `'` (`&#39;`), `"` (`&quot;`), `&` (`&amp;`) e `<>` (`&lt;&gt;`).
  - **Gzip no `/r/`:** com `Accept-Encoding: gzip` a resposta vem `Content-Encoding: gzip`, 720 bytes, e **decodifica para 1471 bytes com o mesmo md5** do corpo sem compressão (`c185196a…`). Confirma que não é o nosso nginx (`gzip off` no bloco): o Supabase/Cloudflare comprime e o nginx repassa, como o adendo do PR #20 já dizia. Sem regressão do PR #20 sobre o `/r/`.
- **Observações, não são regressão:**
  - A **imagem OG desse registro de teste devolve 404** na Amazon (`m.media-amazon.com/.../61Yl5nTbNvL._AC_SL1500_.jpg`): fixture antigo, não é problema de produto. Um link real com imagem válida é que mostra o cartão completo.
  - O `Set-Cookie: __cf_bm=…; Domain=supabase.co` do Cloudflare do Supabase passa pelo nosso nginx no `/r/`. O navegador ignora (domínio diferente do nosso); só registrado.
- **AINDA NÃO PROVADO:** o **cartão desenhado pelo WhatsApp de verdade** — o servidor entrega o HTML certo com o content-type certo, mas quem decide desenhar é o WhatsApp, e isso só o envio real prova. Precisa ser um link **novo** (o WhatsApp guarda em cache a prévia vazia dos antigos; ver REVISÃO 50). O caminho de **gente** (302 + 1 linha em `link_clicks`) também não foi repetido hoje: contaria clique; foi medido na REVISÃO 50.
- **Branch `redesign/megalinks-ui-v3` apagada no remoto** (a pedido do Érico, 19/09): `git merge-base --is-ancestor` confirmou que o topo dela (`c8ff5d5`) era ancestral de `origin/main` e `rev-list origin/main..origin/redesign/megalinks-ui-v3` deu 0 commits exclusivos; depois `git push origin --delete` e `ls-remote` devolveu 0 refs. Os commits seguem na `main` (PR #19, `022f63b`).

**Adendo de 19/09 (5ª parte) — ITENS MENORES: resíduos de paleta em `revops.html` trocados por tokens (só cor, 1 arquivo), bug do alerta de vencimento corrigido, e `.btn.loading` era um item FANTASMA. Verificado no navegador (localhost, dados sintéticos); ainda NÃO em produção enquanto o PR não for mergeado.**

- **`.btn.loading` — não havia CSS morto para remover.** `git log -G'\.btn[^{}]*(loading|busy|spinner)' origin/main -- frontend` devolve **0 commits**: a string nunca esteve no `frontend`. A pendência nasceu como lacuna anotada ("nenhum código aplicaria a classe hoje", REVISÃO 66/handoff) e foi relida depois como "CSS morto". O que existe: `showLoading()`/`hideLoading()` (overlay global `#globalLoading`, `index.html` ~2444), `.btn:disabled` (opacidade) e `@keyframes spin` (`index.html:663`). **Nada foi implementado nem removido.** Se o Érico quiser um estado de carregamento *no botão* (spinner + `disabled` durante a chamada), é feature nova a decidir, não limpeza.
- **O "~7 fundos escuros de badge com hex solto" eram, na verdade, os 7 cabeçalhos de coluna do Kanban** (`KANBAN_COLS`, `revops.html` ~1941: `#2a3a5c`, `#1c2c4d`, `#3a2a16`, `#3a2010`, `#0f3527`, `#3a2a00`, `#3a1c1c`). Agora usam os mesmos tokens das `.tag`, em tinta de 20%: lead neutro (`var(--panel3)`), trial `--info`, expirado e em risco `--brand`, quente `--risk`, cliente `--money`, cancelado `--danger`. Contraste do texto (`--ink`) sobre cada cabeçalho medido: **9,4:1 a 11,5:1** (AA = 4,5:1). Fica mais dessaturado que antes, que é o objetivo da restilização.
- **O achado maior: 13 fundos de badge em `rgba()` das paletas antigas** (`34,197,94`, `245,197,24`, `255,81,104`), em `.tag`, `.iaLabel`, `.kCard .ch`, mais 5 ocorrências soltas (`nav a.on`, o chip "recorrente", o selo do número conectado com `16,185,129` Tailwind, o card de oferta com `255,179,32` e o `.iaLabel.at_risk` com `#ffaa5a`/`255,160,90`). **Por que a passada logada de 19/09 não viu:** o detector só casava `rgb()` exato; `rgba()` com alpha passa batido (limite já anotado no adendo "3ª parte"). Trocados na mesma família de tokens (`--money` 127,185,141 · `--brand` 255,193,7 · `--danger` 226,112,95 · `--risk` 201,161,90), **mantendo o alpha de cada regra**. Varredura final em `revops.html` (hex **e** rgba, inclusive as 7 cores do Kanban): **0 resíduos**.
- **🐛 Bug pré-existente corrigido, com prova:** `renderWaCreditAlert` montava `background:${color}22` (a ideia: sufixo hex de alpha). Com `color='var(--danger)'` isso vira `var(--danger)22`, **CSS inválido, e o navegador descarta o fundo**. Medido: `var(--danger)22` → `backgroundColor: rgba(0, 0, 0, 0)`; `#FFB02022` → `rgba(255,176,32,0.133)`. Ou seja, só a ramificação com hex literal (`#FFB020`, "vence em ≤ 5 dias") tinha o fundo; **"Vencido" e "Em dia" nunca tiveram**. Corrigido com `background:color-mix(in srgb,${color} 13%,transparent)` (13% ≈ o `22` hex), e o `#FFB020` virou `var(--risk)`. Conferido nas três ramificações: fundo com alpha 0,13 na cor do token. **Muda o visual de propósito:** os alertas de vencimento "Vencido" e "Em dia" passam a ter fundo. `color-mix` exige navegador de 2023 em diante (Chrome 111, Safari 16.2, Firefox 113); painel interno e noindex; num navegador antigo o fundo some (o texto continua legível).
- **Verificação (localhost:5180, dados sintéticos, sem login):** `buildKanban()` com 7 usuários de teste; chips de todas as classes alteradas com estilo computado batendo com os tokens; console sem exceção de JS (só 401 de chamadas ao Supabase sem sessão e CORS do `wa-engine` para `localhost`, ambientais, não comparados com a `main`). **Não feito:** captura de tela, `revops.html` logado em produção, overflow (só mudou cor).
- **Fora do escopo, ANOTADO:** `exemplo-configuracao.js` (2×: `rgba(34,197,94,.1)`, `rgba(59,130,246,.1)`) e `exemplo-postagens.js` (1×: `rgba(245,158,11,.1)`) têm 3 callouts inline de tutorial ainda na paleta antiga. Não mexi: não estavam no bilhete. `index.html` e `onboarding.css` só têm `var(--x,#fallback)`, que o handoff manda ignorar. `var(--danger,#c0392b)` e `var(--panel2,#1c1c22)` em `revops.html` também são fallbacks (os tokens existem no `:root`).
- **Lição de ferramenta:** o `preview_start` recusou a porta 5173 (ocupada por outro chat, possivelmente servindo outro worktree). Solução: config local `frontend-static-wt` na porta 5180 em `.claude/launch.json` (o `.claude/` está no `.gitignore`, então não vai para o commit).

### Como retomar esta frente (handoff de 19/09)

> **Atualização pós-merge (19/09):** o PR #19 já foi mergeado (`022f63b`), então o passo 1 abaixo, que compara `origin/main` com `redesign/megalinks-ui-v3`, descreve o estado **antes** do merge. Para a próxima frente, partir da `main` (`git fetch origin && git switch main && git pull`), não da branch de restilização. Os passos 3 a 6 (varreduras de cor, teste sem login, "Em aberto", "Não refazer") continuam valendo. O gzip já foi mergeado (PR #20, `39fa036`) e está no ar. O deploy da `main` é automático.


1. **Primeira ação, sempre:** `git fetch origin` e `git rev-list --left-right --count origin/main...redesign/megalinks-ui-v3`. A `main` andou 15 commits em 3 dias; não confiar no checkout local (P150). Existem **dois clones**: `C:\Users\PC\Documents\megalinksbr` (onde a branch foi feita) e `C:\Users\PC\github\megalinksbr` (na `main`). O `gh` está logado como `rocketdesignbh-dot`.
2. **Se a `main` andou de novo:** `git merge origin/main` na branch. Conflito provável em `frontend/index.html` (áreas mexidas pelos dois lados — na última vez foi o Post Automático) e neste doc (cabeçalho e "Última alteração"; renumerar a revisão). Simular antes com `git merge-tree --write-tree --name-only origin/main redesign/megalinks-ui-v3`.
3. **Depois de integrar, reexecutar as varreduras** — código novo da `main` reintroduz padrões antigos: `rgba()`/hex das paletas retiradas (amarelo `245,197,24`, vermelho `239,68,68`/`255,81,104`, âmbar `245,158,11`/`255,176,32`, verde `34,197,94`, `#ef4444`/`#f59e0b`/`#22c55e` soltos), o plural "canalis" e `#0c0c0e` literal. Conferir **as duas formas** (hex e rgba) e **pular a linha que define o token** (o bug do `--green:var(--green)` nasceu de não pular). Ignorar `var(--x,#fallback)`: o fallback nunca dispara.
4. **Como testar sem login:** `frontend-static` (porta 5173, `.claude/launch.json`). Forçar `authGate.style.display='none'` + `.page.on` mostra o layout de cada tela sem dados; medir `documentElement.scrollWidth - clientWidth` em 320/375/414/768. Smoke do projeto: `node tools/smoke-index.mjs <arquivo>` — baseline **1 erro** (bloco 2, `includes` com regex), o critério é não piorar; comparar contra `git show origin/main:frontend/index.html`.
5. **Em aberto (atualizado na 2ª sessão de 19/09):** (a) **passada logada em produção FEITA em 19/09** (ver o adendo "3ª parte"): 25 telas × desktop e 371 px, `revops.html`, `#modalTicket` com ticket real e a tela de trial (só o template) — sem problema. **Resta:** as 4 páginas puladas (Conexão WhatsApp, IA Insights, Automações, Conexões Admin), o fluxo real do trial encerrado, reduced-motion com a preferência ligada e capturas de tela (o Chrome MCP estourou 4 vezes); "Usuários ativos 0 / MRR R$ 0,00" **está explicado, não é bug** (adendo); (b) `#modalNewPass` continua no modal antigo, de propósito (recuperação de senha não deve fechar por engano); (c) `.btn.loading` — **superado (5ª parte): era item fantasma, o CSS nunca existiu; nada a remover.** Implementar um estado de carregamento no botão seria feature nova; (d) performance: o gzip (`6e56356`) foi **revertido do PR #19** (`07fcbd3`) e virou o **PR #20, mergeado (`39fa036`) e conferido em produção** (`nginx -t` aprovado com nginx 1.31.6 e `Content-Encoding: gzip` medido no ar; ver adendo). **O redirect `/r/` real e a prévia OG foram medidos depois** (adendo "4ª parte"); **resta só** provar o cartão desenhado pelo WhatsApp de verdade, com um link novo enviado numa conversa; `defer` nos scripts do `<head>` não é seguro (ver adendo); (e) ~7 fundos escuros de badge com hex solto em `revops.html` — **feito na 5ª parte** (eram os cabeçalhos do Kanban, mais 13 fundos em `rgba()`; ver o adendo); (f) **PR #19 mergeado** (`022f63b`) **e no ar**: o deploy da `main` é automático (medido em 19/09) e o HTML servido em `/painel/` já traz as mudanças. **A restilização e o `#modalTicket` já foram conferidos rodando em `https://www.megalinksbr.com.br`** (passada logada de 19/09, adendo "3ª parte"). O `main` **local** deste clone está velho (11/08, 900 atrás / 626 à frente): sempre criar branch com `git switch -c <nome> origin/main`, nunca `pull` no `main` local. Como conferir logado: ver "Lições operacionais" no adendo (entrar por `/index.html`, clicar em Admin, login é do Érico).
6. **Não refazer:** a restilização do zero (REVISÃO 63–66 + esta); limpeza geral de emoji (decisão do Érico, REVISÃO 154); migração em massa de paddings inline para `--sp-*` (só declarei a escala, de propósito).
...
Push bloqueado pelo proxy do agente na sessão de nuvem, como nas duas
rodadas anteriores (P144) — patch `0003-post-video-completo.patch` entregue
ao Érico, aplicado com `git am` + `git push` (commit `037878a`), e o Érico
fez o Deploy do serviço `app` no EasyPanel. Verificado via fetch do HTML
servido em `https://www.megalinksbr.com.br/index.html`: os marcadores
`pvMontarTexto`, `pvChkPostCompleto`, `pvPreVisualizarTexto` e
`pvColetarCta` estão todos presentes no HTML de produção — confirma que o
código do modo "post completo" está no ar.

### Confirmação de uso real (Érico, 14/09)

Érico confirmou os dois testes pendentes: (1) o vídeo agendado na REVISÃO
150 chegou certo no grupo do WhatsApp, com legenda e link corretos; (2) o
modo "post completo" (nome/preço/CTA) também funcionou de ponta a ponta —
agendou, o cron processou e a mensagem chegou montada como esperado.
Fecha os itens (a) e (b) da P148. Seguem abertos: testar edição de verdade
(trocar vídeo/grupos de um agendamento existente) e testar cancelamento +
caminhos de falha (grupo sem `group_jid`, sessão desconectada).
...
### Link do Mercado Livre no Post Automático — `send-post` v34 (deploy 69, REVISÃO 167)

- **Produto de ML sai com `meli.la`** (link nativo, endpoint não documentado do painel de Afiliados, cookie `profiles.ml_session_cookie` + Etiqueta ML do dono). Uma chamada por post de ML. Qualquer falha → `matt_*` + `megalinksbr.com.br/r/`.
- **Onde o link nativo do ML existe hoje:** Postar Agora, Link Rápido (`product-search`) e Post Automático (`send-post` v34). **Não** existe no Disparo Manual (`group-blast`) nem no cupom de ML.
- **Se voltar a sair `/r/` para ML:** primeiro olhar o log `[ML][link-nativo] HTTP …` da `send-post` — 401/403 = cookie vencido (Érico recaptura); 400 com `error_code 111` = item inelegível no programa (P153).

### Cupons automáticos dos grupos — `clone-ingest` v35 / `send-post` v33 / página Cupons (REVISÃO 166)

- **No ar:** `clone-ingest` deploy 35 e `send-post` deploy 68, byte a byte iguais ao repo; tabelas `coupon_settings`/`coupon_captures` e bucket `coupon-images`. Painel: no ar a partir do push da REVISÃO 166.
- **Desligado por padrão:** só age para quem ligar em Cupons (por loja, com imagem). Em 25/09 só o Érico ligou (Shopee, 3x, automático).
- **Lojas: Shopee, Amazon, Mercado Livre** (`lojaDoCupom` na `clone-ingest`, `linkAfiliadoCupom` na `send-post`, `CUP_AUTO_SUPORTADAS` no frontend — mudar os três juntos). Imagem recomendada: quadrada 1080×1080, até 5 MB (não há redimensionamento no código). Horários: `SLOTS_CUPOM` na `send-post` e `CUP_AUTO_HORARIOS` no frontend — mudar os dois juntos.
...
| # | Pendência | Origem |
|---|---|---|
| **P168** | 🟡 **Medir o `meli.la` do Post Automático (REVISÃO 167):** próximo produto de ML de um grupo com Post Automático ligado → log `[ML][link-nativo] produto=… -> https://meli.la/…` na `send-post` e o post no grupo com `meli.la` (sem `short_links` novo para ele). Se vier `HTTP 401/403`, recapturar o cookie. Também: "teste cupom" tem um cupom `approved` de 25/09 que não sai porque o Post Automático do grupo está desligado | 25/09 |
| **P166** | 🟡 **Cupons automáticos (REVISÃO 166) — falta medir em produção:** (a) Érico ligar em Cupons (Shopee, imagem, 1–3x, automático/aprovar) e conferir no painel logado que salva e a imagem aparece; (b) primeira captura real em `coupon_captures` + `clone_ingest_log.status like 'cupom_%'`; (c) primeiro cupom saindo pela `send-post` num grupo: `coupon_captures.status='sent'`, imagem chegou e o link abre a campanha com `utm_content=<SubID>-cupom---`. (d) **Decisão pendente do Érico:** a mesma mensagem de um grupo-fonte hoje só alimenta UM grupo de destino (dedupe por `source_msg_id` sem `clone_source_id`) — se quiser dois destinos para o mesmo grupo-fonte, é mudança de índice + `clone-ingest`. (e) **Amazon/ML (seção 5):** primeiro cupom real capturado e enviado; abrir o link e conferir `tag=` (Amazon) / `matt_tool=` (ML) no destino; ML sem nenhum link de cupom visto nos grupos até 25/09 | 25/09 |
| **P167** | 🟡 **Vitrine do ML → produto (`resolve-link` v6) — medir em produção:** em 48h, contar `clone_ingest_log` com `link_path like '/social/%'` (devem cair para só "LISTA do afiliado") contra capturas de ML novas (`store='mercadolivre'` com `salvo`/`publicado`); conferir 1 post real de ML vindo de vitrine com o link abrindo o anúncio certo. Custo: cada vitrine resolvida vira 1 leitura de ML (Scrape.do pessoal ou quota) — acompanhar | 25/09 |
...
| ~~P71~~ | ✅ **FECHADA 15/09 — CONFIRMADO COM ACESSO REAL AO GOOGLE FONTS, FORA DO SANDBOX QUE GEROU A PENDÊNCIA.** `frontend/index.html` do `origin/main` (`a70bcee`) servido num estático local com internet de verdade. `document.fonts.check()` devolveu `true` para Archivo, IBM Plex Sans e IBM Plex Mono (37 fontes carregadas em `document.fonts`), e o `font-family` computado do `h1`/`.auth-h` resolveu para Archivo — não caiu no fallback Helvetica/Arial. Tela de login bateu com o `DIRECAO_VISUAL.md`: raio 2px, sem sombra, log de despacho editorial à esquerda. ⚠️ **O que isto não prova:** que o domínio de produção está servindo o mesmo arquivo, nem a rede do usuário final — isso exige checar `megalinksbr.com.br` direto. Registro original abaixo. ~~🔴 A tipografia da REVISÃO 66 não foi vista por ninguém. O sandbox não alcança o Google Fonts; tudo foi renderizado com fallback. Archivo e IBM Plex Sans/Mono só serão conferidas no navegador depois do Deploy. Sintoma de falha: painel com cara de Arial~~ | 15/09 |
