...
**Adendo de 19/09 (2ª sessão do dia) — PR #19 MERGEADO em `main` (`022f63b`), gzip revertido e aberto à parte como PR #20 (rascunho), `#modalTicket` migrado, conferência logada PARCIAL (commits `6e56356` → revertido em `07fcbd3`, e `b05bafb`).**

- **PR #19 mergeado:** [rocketdesignbh-dot/megalinksbr#19](https://github.com/rocketdesignbh-dot/megalinksbr/pull/19), `redesign/megalinks-ui-v3` → `main`, **merge commit `022f63b` em 19/09 às 14:17 UTC** (35 commits; `mergeable: MERGEABLE`, `CLEAN`, sem checks configurados; a `main` não tinha andado: `0 35` no `rev-list` logo antes). A branch `redesign/megalinks-ui-v3` **não foi apagada** no remoto. **Deploy no EasyPanel não confirmado:** não sei se o push na `main` dispara o rebuild sozinho; o histórico diz que foi o Érico quem rebuildou. Para conferir depois: o HTML servido em `/painel/` deve conter `class="mbg" id="modalTicket"`.
...
- **Performance (etapa 14), MEDIDA em produção:** `GET /painel/` devolve **843.735 bytes sem `Content-Encoding`** (o `nginx:alpine` traz o gzip desligado; `gzip -9` do mesmo arquivo dá ~244 KB). A correção foi escrita em `frontend/Dockerfile` (`6e56356`: `gzip on` para css/js/json/svg/texto no `server`, e `gzip off` em `/r/` para não mexer no que a P60 mediu), mas **foi REVERTIDA antes do merge do PR #19** (`07fcbd3`, decisão do Érico): a **sintaxe do nginx não foi validada** (sem docker/nginx na máquina) e uma config inválida derruba o frontend inteiro no rebuild. **Segue pendente, como PR separado: [#20](https://github.com/rocketdesignbh-dot/megalinksbr/pull/20), aberto como RASCUNHO** (branch `perf/nginx-gzip-frontend`, criada da `main` com `git cherry-pick 6e56356` → `8e2056d`; sem Docker e sem WSL na máquina). **VALIDADO em 19/09** com o **nginx 1.31.6 oficial para Windows** (mesmo número de versão que a produção anuncia em `Server:`), baixado de `nginx.org/download/` (2.797.070 bytes, conferido contra o `Content-Length`) para a pasta temporária da sessão, sem instalar nada no sistema; o `default.conf` foi extraído de dentro do Dockerfile e incluído num `http{}` de teste:
  ...
  - **Falta** conferir `Content-Encoding: gzip` em `/painel/` **em produção** depois do rebuild. **Em produção o painel continua sem compressão** (843.735 bytes por carga) até o #20 ser mergeado e o rebuild rodar.
