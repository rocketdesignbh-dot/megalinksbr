...
5. **Em aberto (atualizado na 2ª sessão de 19/09):** (a) conferir com login real o que **ainda não foi visto** — tela de trial encerrado, `revops.html` logado, telas com dados reais em 375px, `#modalTicket` com ticket real (a Visão Geral do admin já foi vista); ver a observação "Usuários ativos 0 / MRR R$ 0,00" no adendo; (b) `#modalNewPass` continua no modal antigo, de propósito (recuperação de senha não deve fechar por engano); (c) `.btn.loading` — nenhum código aplicaria a classe hoje: implementar o estado nos botões de ação ou remover o CSS morto; (d) performance: o gzip (`6e56356`) foi **revertido do PR #19** (`07fcbd3`) e virou o **PR #20, mergeado (`39fa036`) e conferido em produção** (`nginx -t` aprovado com nginx 1.31.6 e `Content-Encoding: gzip` medido no ar; ver adendo). **Resta só** exercitar um redirect `/r/` real e o preview do WhatsApp, que ficaram sem teste; `defer` nos scripts do `<head>` não é seguro (ver adendo); (e) ~7 fundos escuros de badge com hex solto em `revops.html`; (f) **PR #19 mergeado** (`022f63b`) **e no ar**: o deploy da `main` é automático (medido em 19/09) e o HTML servido em `/painel/` já traz as mudanças. **Ainda não vi a restilização nem o `#modalTicket` rodando em `https://www.megalinksbr.com.br`** — só em `localhost:5173` (branch) e por marcadores no HTML servido; uma passada logada em produção fica em aberto. Como conferir logado: ver "Lições operacionais" no adendo (entrar por `/index.html`, clicar em Admin, login é do Érico).
