...
/**
 * connectSession
 * Cria/recria o socket Baileys de uma sessão.
 * Lida com QR, pareamento e a reconexão obrigatória (código 515) após o 1º pareamento.
 */
async function connectSession(sessionId, authPath, isReconnect = false) {
    const { state, saveCreds } = await useMultiFileAuthState(authPath);
    const { version } = await fetchLatestBaileysVersion();

    if (!isReconnect) {
        console.log(`[QR] Usando WhatsApp Web versão: ${version.join('.')}`);
    }
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        // Novo QR disponível (só na primeira conexão)
        if (qr) {
            try {
        ...
        // Conexão aberta = pareado e pronto
        if (connection === 'open') {
            const phoneNumber = socket.user?.id?.split(':')[0]?.split('@')[0];
            console.log(`[PAIRED] Sessão ${sessionId} conectada: ${phoneNumber}`);
        ...
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.restartRequired) {
                // 515: reinício obrigatório após 1º pareamento → reconectar
                console.log(`[RESTART] Reconectando sessão ${sessionId} após pareamento...`);
                try {
                    await connectSession(sessionId, authPath, true);
                } catch (e) {
            ...
            // outros códigos: deixa a sessão como está (pode ser temporário)
        }
    });
...
app.post('/pair', verifyToken, async (req, res) => {
    try {
        ...
        // Iniciar a sessão (gera QR e lida com reconexão automática)
        await connectSession(sessionId, authPath);

        // Aguardar até 10 segundos pela imagem do QR
        let attempts = 0;
        while (!SESSIONS.get(sessionId)?.qr && attempts < 20) {
