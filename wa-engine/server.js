...
// -- ML Search Proxy --
// Browser nao consegue chamar api.mercadolibre.com diretamente (CORS)
// Este endpoint faz a busca pelo VPS (IP nao bloqueado pelo ML)
app.post('/ml-search', verifyToken, async (req, res) => {
    ...
    await Promise.all(kws.map(async (kw) => {
        try {
            const url = `https://api.mercadolibre.com/sites/MLB/search?q=${encodeURIComponent(kw)}&sort=relevance&limit=${limit}`;
            const r = await axios.get(url, {
                headers: { 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0' },
                timeout: 8000
            });
            const items = (r.data.results || []).slice(0, 15);
            for (const item of items) {
                const price = Number(item.price || 0);
                const original = item.original_price ? Number(item.original_price) : 0;
                const discPct = original > price && original > 0 ? Math.round((1 - price / original) * 100) : 0;
                if (!item.id || !item.title || price <= 0) continue;
                results.push({
                    source: 'mercado_livre',
                    item_id: item.id,
                    shop_id: String(item.seller?.id || ''),
                    title: item.title,
                    keyword: kw,
                    ...
                    score: Math.round((Math.min(discPct, 90) / 90 * 0.45 + Math.min(Number(item.sold_quantity || 0), 10000) / 10000 * 0.2 + 0.35) * 100),
