 ...
 * Suporta código de pareamento (preferido) e QR (fallback).
 * Lida com a reconexão obrigatória (código 515) após o 1º pareamento.
 */
async function connectSession(sessionId, authPath, phoneNumber = null, isReconnect = false) {
    ...
    const socket = makeWASocket({
        ...
        qrTimeout: 40000, // cada QR vale ~40s (dá mais folga pra escanear)
    });

    socket.ev.on('creds.update', saveCreds);

    // Garantir que a sessão exista no mapa desde já
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.restartRequired) {
                ...
                // QR expirou sem ninguém escanear — marca pra UI avisar
                const s = SESSIONS.get(sessionId);
