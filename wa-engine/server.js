...
// ── /ml-product  (busca dados de produto único via Scrape.do) ──────────────
app.get('/ml-product', verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (!r.ok) return res.status(502).json({ ok: false, error: `Scrape.do HTTP ${r.status}` });
        ...
        if (!title) {
            return res.json({ ok: false, error: 'Produto não encontrado (antibot). Preencha manualmente.' });
        }
