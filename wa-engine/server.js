/**
 * Mega Links BR - WhatsApp Engine  v2
 * Motor de automação WhatsApp baseado em Baileys
 * 
 * MUDANÇAS v2:
 * - Sessões persistem indefinidamente (sem expiração de 6h)
 * - Auto-restauração de sessões salvas no startup
 * - Reconexão automática em qualquer desconexão (exceto loggedOut)
 * - Backoff exponencial em reconexões repetidas
 * - Endpoint GET /reconnect/:phone para forçar reconexão do frontend
 * 
...
async function restoreSessions() {
    try {
        ...
        for (const dir of dirs) {
            ...
            // Verifica se tem credenciais (creds.json é o arquivo principal do Baileys)
            const credsFile = path.join(authPath, 'creds.json');
            if (!await fs.pathExists(credsFile)) {
                console.log(`[RESTORE] ${dir} — sem creds.json, pulando`);
                continue;
            }

            // Lê metadados se disponíveis
            const metaFile = path.join(authPath, '_meta.json');
            let meta = {};
            ...
            console.log(`[RESTORE] Reconectando sessão ${sessionId}${phone ? ' (phone ' + phone + ')' : ''}...`);
            ...
            try {
                ...
                console.error(`[RESTORE] Falha ao reconectar ${sessionId}:`, e.message);
            }
        }

        console.log(`[RESTORE] ${restored} sessão(ões) restaurada(s) de ${dirs.length} pasta(s)`);
    } catch (e) {
...
async function startup() {
    try {
        ...
        app.listen(PORT, () => {
            console.log(`
╔════════════════════════════════════════╗
║  🚀 Mega Links BR - wa-engine  v2      ║
║  Porta ${PORT} · Sessões persistentes     ║
╚════════════════════════════════════════╝
